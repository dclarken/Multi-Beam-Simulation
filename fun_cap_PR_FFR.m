function[capcity]=fun_cap_PR_FFR(PR,FR)%没有迭代次数 第一次分配功率 求得的系统容量
B_tot=2;%下行总带宽为2G
K=24;%波束个数
Q=32;%分配的载波个数为32
B_c=B_tot/Q;%每个载波分配的带宽20.4082M(96)
%PR=x;%主载波和辅载波的功率比
%FR=1/5;中心波束的频带宽度比上总带宽.外层波束相当于还是4色复用，
...
for i=1:K
    ...
    U_out(i)=W_out(i)+H(i,i);
    w_out=zeros(1,K);%外层波束信号同频干扰因子
    w_in=ones(1,K);%内层波束信号同频干扰因子
    for j=1:K/4
        if (i==red(j)) w_out=wred;
        elseif (i==blue(j)) w_out=wblue;
        elseif (i==black(j)) w_out=wblack;
        elseif (i==yellow(j)) w_out=wyellow;
        end
    end
%     w_in=w_out;
    intf_out=0;
    intf_in=0;
    h_in(i,:)=w_in.*(H(i,:)+t*W_in(i));%在求解信噪比时添加参数t减缓信噪比恶化情况
    h_out(i,:)=w_out.*(H(i,:)+t*W_out(i));
    for m=1:K
        intf_in=intf_in+h_in(i,m);
        intf_out=intf_out+h_out(i,m);
    end
    I(i)=u*(intf_in-(W_in(i)+H(i,i)))+v;
    I_out(i)=u*(intf_out-(W_out(i)+H(i,i)))+v;
    CI_in(i)=U(i)-I(i);%信噪比 主
    CI_out(i)= U_out(i)-I_out(i);%信噪比 子 主子信道功率相同的容量分析
end
%capcity
capcity=0;
for i=1:24
